---
title: "Use Cases for the Cry and rgl packages: SAED, Crystal Structure Visualization, and PXRD Pattern Simulation"
output:
  html_document:
    toc: true
    toc_float: true
...
This package provides tools for selected area electron diffraction (SAED) pattern and crystal structure visualization by using ``rgl`` and ``cry`` packages.
In particular ``cry_demo()`` and ``dp_demo()`` read a file in CIF formats (Crystallographic Information Framework) and display SAED pattern and crystal structure.
The function ``dp_demo()`` also performs simple simulation of powder X-ray diffraction (PXRD) pattern and it can be saved the results to a file in the working directory.
...
In this example, a file is not specified.  Therefore, a system default is used.
```{r, eval=FALSE, echo=TRUE}
dp_demo()
```
...
The following is the working snapshot.
Note that while this WebGL graphics is interactive, the behavior is simulated using JavaScript instead of processing in actual R.
...
rglwidget(scene1, width = 0.4 * figWidth(), height = 0.4 * figWidth())
...
rglwidget(width = 0.4 * figWidth(), height = 0.4 * figWidth())
##rglwidget(scene2, width = 0.4 * figWidth(), height = 0.4 * figWidth())
```
</div>
...
The result is putted as a file on the current directory by specifying options like this:
```{r, eval=FALSE, echo=TRUE}
...
```{r, echo=TRUE, fig.asp=0.5, out.width="90%", fig.align="center", fig.cap=""}
f1 <- function(x) { # pseudo-Voigt function
  w <- 0.005 # hwhm (= fwhm/2)
  r <- 0.01 # ratio of Lorentzian and Gaussian
  sum(apply(df2, 1, function(v) {
    h <- v["I"]
    t <- v["twotheta"]
...
x <- do.call(c, sapply(df2$twotheta, function(v) {
  list(v - 0.02, v - 0.01, v, v + 0.01, v + 0.02)
}))
